// SPDX-FileCopyrightText: 2024 Intel Corporation
//
// SPDX-License-Identifier: Apache-2.0
//
// Specification of the 3GPP Confidentiality and Integrity Algorithms 128-EEA3 & 128-EIA3.
// Document 3: Implementor's Test Data
// https://www.gsma.com/security/wp-content/uploads/2019/05/eea3eia3testdatav11.pdf
//

package security

import (
	"bytes"
	"crypto/aes"
	"encoding/hex"
	"testing"
)

// TestAESCMACSumRFC4493 validates aesCMACSum against all four test vectors from
// RFC 4493 (https://www.rfc-editor.org/rfc/rfc4493), Section 4, using the same
// 128-bit key. The vectors exercise: empty message, one exact block, partial
// multi-block, and multiple exact blocks.
func TestAESCMACSumRFC4493(t *testing.T) {
	t.Parallel()

	const keyHex = "2b7e151628aed2a6abf7158809cf4f3c" // gitleaks:allow

	testCases := []struct {
		name    string
		msgHex  string
		wantHex string
	}{
		{
			// Example 1: empty message
			name:    "EmptyMessage",
			msgHex:  "",
			wantHex: "bb1d6929e95937287fa37d129b756746",
		},
		{
			// Example 2: exactly one 16-byte block (complete block → uses K1)
			name:    "OneCompleteBlock",
			msgHex:  "6bc1bee22e409f96e93d7e117393172a",
			wantHex: "070a16b46b4d4144f79bdd9dd04a287c",
		},
		{
			// Example 3: 40 bytes – two complete blocks + one partial block (uses K2)
			name:    "PartialLastBlock",
			msgHex:  "6bc1bee22e409f96e93d7e117393172aae2d8a571e03ac9c9eb76fac45af8e5130c81c46a35ce411",
			wantHex: "dfa66747de9ae63030ca32611497c827",
		},
		{
			// Example 4: 64 bytes – four complete blocks (uses K1)
			name:    "MultipleCompleteBlocks",
			msgHex:  "6bc1bee22e409f96e93d7e117393172aae2d8a571e03ac9c9eb76fac45af8e5130c81c46a35ce411e5fbc1191a0a52eff69f2445df4f9b17ad2b417be66c3710",
			wantHex: "51f0bebf7e3b9d92fc49741779363cfe",
		},
	}

	key, err := hex.DecodeString(keyHex)
	if err != nil {
		t.Fatalf("decode key: %v", err)
	}

	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			var msg []byte
			if tc.msgHex != "" {
				msg, err = hex.DecodeString(tc.msgHex)
				if err != nil {
					t.Fatalf("decode message: %v", err)
				}
			}

			want, err := hex.DecodeString(tc.wantHex)
			if err != nil {
				t.Fatalf("decode expected MAC: %v", err)
			}

			block, err := aes.NewCipher(key)
			if err != nil {
				t.Fatalf("create cipher: %v", err)
			}

			got, err := aesCMACSum(block, msg)
			if err != nil {
				t.Fatalf("compute CMAC: %v", err)
			}
			if !bytes.Equal(want, got) {
				t.Errorf("CMAC mismatch: expected %x, got %x", want, got)
			}
		})
	}
}

// TestNEA1 validates 128-EEA1 (SNOW3G-based confidentiality) using test vectors
// from 3GPP TS 33.401 Annex C.3.
// NOTE: The snow3g package uses package-level global state, so this test must
// NOT run in parallel with other tests that call SNOW3G functions.
func TestNEA1(t *testing.T) {
	testSets := []struct {
		name       string
		key        [16]byte
		countC     uint32
		bearer     uint32
		direction  uint32
		length     uint32
		plainText  []byte
		cipherText []byte
	}{
		{
			// Test Set 1 – 798 bits
			name:      "NEA1TestSet1",
			key:       [16]byte{0x2b, 0xd6, 0x45, 0x9f, 0x82, 0xc5, 0xb3, 0x00, 0x95, 0x2c, 0x49, 0x10, 0x48, 0x81, 0xff, 0x48},
			countC:    0x72a4f20f,
			bearer:    0x0c,
			direction: 1,
			length:    798,
			plainText: []byte{
				0x7e, 0xc6, 0x12, 0x72, 0x74, 0x3b, 0xf1, 0x61, 0x47, 0x26, 0x44, 0x6a, 0x6c, 0x38, 0xce, 0xd1,
				0x66, 0xf6, 0xca, 0x76, 0xeb, 0x54, 0x30, 0x04, 0x42, 0x86, 0x34, 0x6c, 0xef, 0x13, 0x0f, 0x92,
				0x92, 0x2b, 0x03, 0x45, 0x0d, 0x3a, 0x99, 0x75, 0xe5, 0xbd, 0x2e, 0xa0, 0xeb, 0x55, 0xad, 0x8e,
				0x1b, 0x19, 0x9e, 0x3e, 0xc4, 0x31, 0x60, 0x20, 0xe9, 0xa1, 0xb2, 0x85, 0xe7, 0x62, 0x79, 0x53,
				0x59, 0xb7, 0xbd, 0xfd, 0x39, 0xbe, 0xf4, 0xb2, 0x48, 0x45, 0x83, 0xd5, 0xaf, 0xe0, 0x82, 0xae,
				0xe6, 0x38, 0xbf, 0x5f, 0xd5, 0xa6, 0x06, 0x19, 0x39, 0x01, 0xa0, 0x8f, 0x4a, 0xb4, 0x1a, 0xab,
				0x9b, 0x13, 0x48, 0x80,
			},
			cipherText: []byte{
				0x8c, 0xeb, 0xa6, 0x29, 0x43, 0xdc, 0xed, 0x3a, 0x09, 0x90, 0xb0, 0x6e, 0xa1, 0xb0, 0xa2, 0xc4,
				0xfb, 0x3c, 0xed, 0xc7, 0x1b, 0x36, 0x9f, 0x42, 0xba, 0x64, 0xc1, 0xeb, 0x66, 0x65, 0xe7, 0x2a,
				0xa1, 0xc9, 0xbb, 0x0d, 0xea, 0xa2, 0x0f, 0xe8, 0x60, 0x58, 0xb8, 0xba, 0xee, 0x2c, 0x2e, 0x7f,
				0x0b, 0xec, 0xce, 0x48, 0xb5, 0x29, 0x32, 0xa5, 0x3c, 0x9d, 0x5f, 0x93, 0x1a, 0x3a, 0x7c, 0x53,
				0x22, 0x59, 0xaf, 0x43, 0x25, 0xe2, 0xa6, 0x5e, 0x30, 0x84, 0xad, 0x5f, 0x6a, 0x51, 0x3b, 0x7b,
				0xdd, 0xc1, 0xb6, 0x5f, 0x0a, 0xa0, 0xd9, 0x7a, 0x05, 0x3d, 0xb5, 0x5a, 0x88, 0xc4, 0xc4, 0xf9,
				0x60, 0x5e, 0x41, 0x40,
			},
		},
		{
			// Test Set 2 – 510 bits (64 bytes, non-zero remainder)
			name:      "NEA1TestSet2",
			key:       [16]byte{0xef, 0xa8, 0xb2, 0x22, 0x9e, 0x72, 0x0c, 0x2a, 0x7c, 0x36, 0xea, 0x55, 0xe9, 0x60, 0x56, 0x95},
			countC:    0xe28bcf7b,
			bearer:    0x18,
			direction: 0,
			length:    510,
			plainText: []byte{
				0x10, 0x11, 0x12, 0x31, 0xe0, 0x60, 0x25, 0x3a, 0x43, 0xfd, 0x3f, 0x57, 0xe3, 0x76, 0x07, 0xab,
				0x28, 0x27, 0xb5, 0x99, 0xb6, 0xb1, 0xbb, 0xda, 0x37, 0xa8, 0xab, 0xcc, 0x5a, 0x8c, 0x55, 0x0d,
				0x1b, 0xfb, 0x2f, 0x49, 0x46, 0x24, 0xfb, 0x50, 0x36, 0x7f, 0xa3, 0x6c, 0xe3, 0xbc, 0x68, 0xf1,
				0x1c, 0xf9, 0x3b, 0x15, 0x10, 0x37, 0x6b, 0x02, 0x13, 0x0f, 0x81, 0x2a, 0x9f, 0xa1, 0x69, 0xd8,
			},
			cipherText: []byte{
				0xe0, 0xda, 0x15, 0xca, 0x8e, 0x25, 0x54, 0xf5, 0xe5, 0x6c, 0x94, 0x68, 0xdc, 0x6c, 0x7c, 0x12,
				0x9c, 0x56, 0x8a, 0xa5, 0x03, 0x23, 0x17, 0xe0, 0x4e, 0x07, 0x29, 0x64, 0x6c, 0xab, 0xef, 0xa6,
				0x89, 0x86, 0x4c, 0x41, 0x0f, 0x24, 0xf9, 0x19, 0xe6, 0x1e, 0x3d, 0xfd, 0xfa, 0xd7, 0x7e, 0x56,
				0x0d, 0xb0, 0xa9, 0xcd, 0x36, 0xc3, 0x4a, 0xe4, 0x18, 0x14, 0x90, 0xb2, 0x9f, 0x5f, 0xa2, 0xfc,
			},
		},
		{
			// Test Set 3 – 120 bits (15 bytes)
			name:       "NEA1TestSet3",
			key:        [16]byte{0x5a, 0xcb, 0x1d, 0x64, 0x4c, 0x0d, 0x51, 0x20, 0x4e, 0xa5, 0xf1, 0x45, 0x10, 0x10, 0xd8, 0x52},
			countC:     0xfa556b26,
			bearer:     0x03,
			direction:  1,
			length:     120,
			plainText:  []byte{0xad, 0x9c, 0x44, 0x1f, 0x89, 0x0b, 0x38, 0xc4, 0x57, 0xa4, 0x9d, 0x42, 0x14, 0x07, 0xe8, 0x00},
			cipherText: []byte{0xba, 0x0f, 0x31, 0x30, 0x03, 0x34, 0xc5, 0x6b, 0x52, 0xa7, 0x49, 0x7c, 0xba, 0xc0, 0x46, 0x00},
		},
		{
			// Test Set 4 – 253 bits (32 bytes)
			name:      "NEA1TestSet4",
			key:       [16]byte{0xd3, 0xc5, 0xd5, 0x92, 0x32, 0x7f, 0xb1, 0x1c, 0x40, 0x35, 0xc6, 0x68, 0x0a, 0xf8, 0xc6, 0xd1},
			countC:    0x398a59b4,
			bearer:    0x05,
			direction: 1,
			length:    253,
			plainText: []byte{
				0x98, 0x1b, 0xa6, 0x82, 0x4c, 0x1b, 0xfb, 0x1a, 0xb4, 0x85, 0x47, 0x20, 0x29, 0xb7, 0x1d, 0x80,
				0x8c, 0xe3, 0x3e, 0x2c, 0xc3, 0xc0, 0xb5, 0xfc, 0x1f, 0x3d, 0xe8, 0xa6, 0xdc, 0x66, 0xb1, 0xf0,
			},
			cipherText: []byte{
				0x98, 0x9b, 0x71, 0x9c, 0xdc, 0x33, 0xce, 0xb7, 0xcf, 0x27, 0x6a, 0x52, 0x82, 0x7c, 0xef, 0x94,
				0xa5, 0x6c, 0x40, 0xc0, 0xab, 0x9d, 0x81, 0xf7, 0xa2, 0xa9, 0xba, 0xc6, 0x0e, 0x11, 0xc4, 0xb0,
			},
		},
	}

	for _, ts := range testSets {
		t.Run(ts.name, func(t *testing.T) {
			cipher, err := NEA1(ts.key, ts.countC, ts.bearer, ts.direction, ts.plainText, ts.length)
			if err != nil {
				t.Errorf("error: %v", err)
			}
			if !bytes.Equal(ts.cipherText, cipher) {
				t.Errorf("ciphertext mismatch: expected %x, got %x", ts.cipherText, cipher)
			}
		})
	}
}

// TestNIA1 validates 128-EIA1 (SNOW3G-based integrity) using test vectors
// from 3GPP TS 33.401 Annex C.4.
// NOTE: The snow3g package uses package-level global state, so this test must
// NOT run in parallel with other tests that call SNOW3G functions.
func TestNIA1(t *testing.T) {
	testSets := []struct {
		name      string
		key       [16]byte
		countI    uint32
		bearer    byte
		direction uint32
		length    uint64
		message   []byte
		mac       []byte
	}{
		{
			// Test Set 1 – 88 bits
			name:      "NIA1TestSet1",
			key:       [16]byte{0x2b, 0xd6, 0x45, 0x9f, 0x82, 0xc5, 0xb3, 0x00, 0x95, 0x2c, 0x49, 0x10, 0x48, 0x81, 0xff, 0x48},
			countI:    0x38a6f056,
			bearer:    0x1f,
			direction: 0,
			length:    88,
			message: []byte{
				0x33, 0x32, 0x34, 0x62, 0x63, 0x39, 0x38, 0x61, 0x37, 0x34, 0x79, 0x00,
				0x00, 0x00, 0x00, 0x00,
			},
			mac: []byte{0x73, 0x1f, 0x11, 0x65},
		},
		{
			// Test Set 2 – 254 bits (32 bytes)
			name:      "NIA1TestSet2",
			key:       [16]byte{0x7e, 0x5e, 0x94, 0x43, 0x1e, 0x11, 0xd7, 0x38, 0x28, 0xd7, 0x39, 0xcc, 0x6c, 0xed, 0x45, 0x73},
			countI:    0x36af6144,
			bearer:    0x18,
			direction: 1,
			length:    254,
			message: []byte{
				0xb3, 0xd3, 0xc9, 0x17, 0x0a, 0x4e, 0x16, 0x32, 0xf6, 0x0f, 0x86, 0x10, 0x13, 0xd2, 0x2d, 0x84,
				0xb7, 0x26, 0xb6, 0xa2, 0x78, 0xd8, 0x02, 0xd1, 0xee, 0xaf, 0x13, 0x21, 0xba, 0x59, 0x29, 0xdc,
			},
			mac: []byte{0xe3, 0x25, 0x9f, 0x6f},
		},
		{
			// Test Set 3 – 511 bits
			name:      "NIA1TestSet3",
			key:       [16]byte{0xd3, 0x41, 0x9b, 0xe8, 0x21, 0x08, 0x7a, 0xcd, 0x02, 0x12, 0x3a, 0x92, 0x48, 0x03, 0x33, 0x59},
			countI:    0xc7590ea9,
			bearer:    0x17,
			direction: 0,
			length:    511,
			message: []byte{
				0xbb, 0xb0, 0x57, 0x03, 0x88, 0x09, 0x49, 0x6b, 0xcf, 0xf8, 0x6d, 0x6f, 0xbc, 0x8c, 0xe5, 0xb1,
				0x35, 0xa0, 0x6b, 0x16, 0x60, 0x54, 0xf2, 0xd5, 0x65, 0xbe, 0x8a, 0xce, 0x75, 0xdc, 0x85, 0x1e,
				0x0b, 0xcd, 0xd8, 0xf0, 0x71, 0x41, 0xc4, 0x95, 0x87, 0x2f, 0xb5, 0xd8, 0xc0, 0xc6, 0x6a, 0x8b,
				0x6d, 0xa5, 0x56, 0x66, 0x3e, 0x4e, 0x46, 0x12, 0x05, 0xd8, 0x45, 0x80, 0xbe, 0xe5, 0xbc, 0x7e,
			},
			mac: []byte{0x9a, 0x16, 0xc7, 0x7d},
		},
		{
			// Test Set 4 – 768 bits
			name:      "NIA1TestSet4",
			key:       [16]byte{0x83, 0xfd, 0x23, 0xa2, 0x44, 0xa7, 0x4c, 0xf3, 0x58, 0xda, 0x30, 0x19, 0xf1, 0x72, 0x26, 0x35},
			countI:    0x36af6144,
			bearer:    0x0f,
			direction: 1,
			length:    768,
			message: []byte{
				0x35, 0xc6, 0x87, 0x16, 0x63, 0x3c, 0x66, 0xfb, 0x75, 0x0c, 0x26, 0x68, 0x65, 0xd5, 0x3c, 0x11,
				0xea, 0x05, 0xb1, 0xe9, 0xfa, 0x49, 0xc8, 0x39, 0x8d, 0x48, 0xe1, 0xef, 0xa5, 0x90, 0x9d, 0x39,
				0x47, 0x90, 0x28, 0x37, 0xf5, 0xae, 0x96, 0xd5, 0xa0, 0x5b, 0xc8, 0xd6, 0x1c, 0xa8, 0xdb, 0xef,
				0x1b, 0x13, 0xa4, 0xb4, 0xab, 0xfe, 0x4f, 0xb1, 0x00, 0x60, 0x45, 0xb6, 0x74, 0xbb, 0x54, 0x72,
				0x93, 0x04, 0xc3, 0x82, 0xbe, 0x53, 0xa5, 0xaf, 0x05, 0x55, 0x61, 0x76, 0xf6, 0xea, 0xa2, 0xef,
				0x1d, 0x05, 0xe4, 0xb0, 0x83, 0x18, 0x1e, 0xe6, 0x74, 0xcd, 0xa5, 0xa4, 0x85, 0xf7, 0x4d, 0x7a,
			},
			mac: []byte{0xbb, 0xa7, 0x44, 0x92},
		},
		{
			// Test Set 5 – 383 bits
			name:      "NIA1TestSet5",
			key:       [16]byte{0x68, 0x32, 0xa6, 0x5c, 0xff, 0x44, 0x73, 0x62, 0x1e, 0xbd, 0xd4, 0xba, 0x26, 0xa9, 0x21, 0xfe},
			countI:    0x36af6144,
			bearer:    0x18,
			direction: 0,
			length:    383,
			message: []byte{
				0xd3, 0xc5, 0x38, 0x39, 0x62, 0x68, 0x20, 0x71, 0x77, 0x65, 0x66, 0x76, 0x20, 0x32, 0x38, 0x37,
				0x63, 0x62, 0x40, 0x98, 0x1b, 0xa6, 0x82, 0x4c, 0x1b, 0xfb, 0x1a, 0xb4, 0x85, 0x47, 0x20, 0x29,
				0xb7, 0x1d, 0x80, 0x8c, 0xe3, 0x3e, 0x2c, 0xc3, 0xc0, 0xb5, 0xfc, 0x1f, 0x3d, 0xe8, 0xa6, 0xdc,
			},
			mac: []byte{0x41, 0x45, 0xe4, 0xb0},
		},
		{
			// Test Set 6 – 2558 bits (320 bytes)
			name:      "NIA1TestSet6",
			key:       [16]byte{0x5d, 0x0a, 0x80, 0xd8, 0x13, 0x4a, 0xe1, 0x96, 0x77, 0x82, 0x4b, 0x67, 0x1e, 0x83, 0x8a, 0xf4},
			countI:    0x7827fab2,
			bearer:    0x05,
			direction: 1,
			length:    2558,
			message: []byte{
				,
			},
			mac: []byte{0x0f, 0xa2, 0xb1, 0xee},
		},
		{
			// Test Set 7 – 16448 bits (2056 bytes)
			name:      "NIA1TestSet7",
			key:       [16]byte{0xb3, 0x12, 0x0f, 0xfd, 0xb2, 0xcf, 0x6a, 0xf4, 0xe7, 0x3e, 0xaf, 0x2e, 0xf4, 0xeb, 0xec, 0x69},
			countI:    0x296f393c,
			bearer:    0x0b,
			direction: 1,
			length:    16448,
			message: []byte{
				,
			},
			mac: []byte{0xab, 0xf3, 0xe6, 0x51},
		},
	}

	for _, ts := range testSets {
		t.Run(ts.name, func(t *testing.T) {
			output, err := NIA1(ts.key, ts.countI, ts.bearer, ts.direction, ts.message, ts.length)
			if err != nil {
				t.Errorf("error: %v", err)
			}
			if !bytes.Equal(ts.mac, output) {
				t.Errorf("MAC mismatch: expected %x, got %x", ts.mac, output)
			}
		})
	}
}

// TestNEA2 validates 128-EEA2 (AES-CTR-based confidentiality) using test vectors
// from 3GPP TS 33.401 Annex C.1.
func TestNEA2(t *testing.T) {
	t.Parallel()

	testSets := []struct {
		name       string
		key        [16]byte
		count      uint32
		bearer     uint8
		direction  uint8
		plainText  []byte
		cipherText []byte
	}{
		{
			// Test Set 1 – 248 bits (31 bytes)
			name:      "NEA2TestSet1",
			key:       [16]byte{0xd3, 0xc5, 0xd5, 0x92, 0x32, 0x7f, 0xb1, 0x1c, 0x40, 0x35, 0xc6, 0x68, 0x0a, 0xf8, 0xc6, 0xd1},
			count:     0x398a59b4,
			bearer:    0x15,
			direction: 1,
			plainText: []byte{
				0x98, 0x1b, 0xa6, 0x82, 0x4c, 0x1b, 0xfb, 0x1a, 0xb4, 0x85, 0x47, 0x20, 0x29, 0xb7, 0x1d, 0x80,
				0x8c, 0xe3, 0x3e, 0x2c, 0xc3, 0xc0, 0xb5, 0xfc, 0x1f, 0x3d, 0xe8, 0xa6, 0xdc, 0x66, 0xb1,
			},
			cipherText: []byte{
				0xe9, 0xfe, 0xd8, 0xa6, 0x3d, 0x15, 0x53, 0x04, 0xd7, 0x1d, 0xf2, 0x0b, 0xf3, 0xe8, 0x22, 0x14,
				0xb2, 0x0e, 0xd7, 0xda, 0xd2, 0xf2, 0x33, 0xdc, 0x3c, 0x22, 0xd7, 0xbd, 0xee, 0xed, 0x8e,
			},
		},
		{
			// Test Set 2 – 798 bits (99 bytes)
			name:      "NEA2TestSet2",
			key:       [16]byte{0x2b, 0xd6, 0x45, 0x9f, 0x82, 0xc4, 0x40, 0xe0, 0x95, 0x2c, 0x49, 0x10, 0x48, 0x05, 0xff, 0x48},
			count:     0xc675a64b,
			bearer:    0x0c,
			direction: 1,
			plainText: []byte{
				0x7e, 0xc6, 0x12, 0x72, 0x74, 0x3b, 0xf1, 0x61, 0x47, 0x26, 0x44, 0x6a, 0x6c, 0x38, 0xce, 0xd1,
				0x66, 0xf6, 0xca, 0x76, 0xeb, 0x54, 0x30, 0x04, 0x42, 0x86, 0x34, 0x6c, 0xef, 0x13, 0x0f, 0x92,
				0x92, 0x2b, 0x03, 0x45, 0x0d, 0x3a, 0x99, 0x75, 0xe5, 0xbd, 0x2e, 0xa0, 0xeb, 0x55, 0xad, 0x8e,
				0x1b, 0x19, 0x9e, 0x3e, 0xc4, 0x31, 0x60, 0x20, 0xe9, 0xa1, 0xb2, 0x85, 0xe7, 0x62, 0x79, 0x53,
				0x59, 0xb7, 0xbd, 0xfd, 0x39, 0xbe, 0xf4, 0xb2, 0x48, 0x45, 0x83, 0xd5, 0xaf, 0xe0, 0x82, 0xae,
				0xe6, 0x38, 0xbf, 0x5f, 0xd5, 0xa6, 0x06, 0x19, 0x39, 0x01, 0xa0, 0x8f, 0x4a, 0xb4, 0x1a, 0xab,
				0x9b, 0x13, 0x48,
			},
			cipherText: []byte{
				0x59, 0x61, 0x60, 0x53, 0x53, 0xc6, 0x4b, 0xdc, 0xa1, 0x5b, 0x19, 0x5e, 0x28, 0x85, 0x53, 0xa9,
				0x10, 0x63, 0x25, 0x06, 0xd6, 0x20, 0x0a, 0xa7, 0x90, 0xc4, 0xc8, 0x06, 0xc9, 0x99, 0x04, 0xcf,
				0x24, 0x45, 0xcc, 0x50, 0xbb, 0x1c, 0xf1, 0x68, 0xa4, 0x96, 0x73, 0x73, 0x4e, 0x08, 0x1b, 0x57,
				0xe3, 0x24, 0xce, 0x52, 0x59, 0xc0, 0xe7, 0x8d, 0x4c, 0xd9, 0x7b, 0x87, 0x09, 0x76, 0x50, 0x3c,
				0x09, 0x43, 0xf2, 0xcb, 0x5a, 0xe8, 0xf0, 0x52, 0xc7, 0xb7, 0xd3, 0x92, 0x23, 0x95, 0x87, 0xb8,
				0x95, 0x60, 0x86, 0xbc, 0xab, 0x18, 0x83, 0x60, 0x42, 0xe2, 0xe6, 0xce, 0x42, 0x43, 0x2a, 0x17,
				0x10, 0x5c, 0x53,
			},
		},
		{
			// Test Set 3 – 304 bits (38 bytes)
			name:      "NEA2TestSet3",
			key:       [16]byte{0x0a, 0x8b, 0x6b, 0xd8, 0xd9, 0xb0, 0x8b, 0x08, 0xd6, 0x4e, 0x32, 0xd1, 0x81, 0x77, 0x77, 0xfb},
			count:     0x544d49cd,
			bearer:    0x04,
			direction: 0,
			plainText: []byte{
				0xfd, 0x40, 0xa4, 0x1d, 0x37, 0x0a, 0x1f, 0x65, 0x74, 0x50, 0x95, 0x68, 0x7d, 0x47, 0xba, 0x1d,
				0x36, 0xd2, 0x34, 0x9e, 0x23, 0xf6, 0x44, 0x39, 0x2c, 0x8e, 0xa9, 0xc4, 0x9d, 0x40, 0xc1, 0x32,
				0x71, 0xaf, 0xf2, 0x64, 0xd0, 0xf2,
			},
			cipherText: []byte{
				0x75, 0x75, 0x0d, 0x37, 0xb4, 0xbb, 0xa2, 0xa4, 0xde, 0xdb, 0x34, 0x23, 0x5b, 0xd6, 0x8c, 0x66,
				0x45, 0xac, 0xda, 0xac, 0xa4, 0x81, 0x38, 0xa3, 0xb0, 0xc4, 0x71, 0xe2, 0xa7, 0x04, 0x1a, 0x57,
				0x64, 0x23, 0xd2, 0x92, 0x72, 0x87,
			},
		},
		{
			// Test Set 4 – 1016 bits (127 bytes)
			name:      "NEA2TestSet4",
			key:       [16]byte{0xaa, 0x1f, 0x95, 0xae, 0xa5, 0x33, 0xbc, 0xb3, 0x2e, 0xb6, 0x3b, 0xf5, 0x2d, 0x8f, 0x83, 0x1a},
			count:     0x72d8c671,
			bearer:    0x10,
			direction: 1,
			plainText: []byte{
				0xfb, 0x1b, 0x96, 0xc5, 0xc8, 0xba, 0xdf, 0xb2, 0xe8, 0xe8, 0xed, 0xfd, 0xe7, 0x8e, 0x57, 0xf2,
				0xad, 0x81, 0xe7, 0x41, 0x03, 0xfc, 0x43, 0x0a, 0x53, 0x4d, 0xcc, 0x37, 0xaf, 0xce, 0xc7, 0x0e,
				0x15, 0x17, 0xbb, 0x06, 0xf2, 0x72, 0x19, 0xda, 0xe4, 0x90, 0x22, 0xdd, 0xc4, 0x7a, 0x06, 0x8d,
				0xe4, 0xc9, 0x49, 0x6a, 0x95, 0x1a, 0x6b, 0x09, 0xed, 0xbd, 0xc8, 0x64, 0xc7, 0xad, 0xbd, 0x74,
				0x0a, 0xc5, 0x0c, 0x02, 0x2f, 0x30, 0x82, 0xba, 0xfd, 0x22, 0xd7, 0x81, 0x97, 0xc5, 0xd5, 0x08,
				0xb9, 0x77, 0xbc, 0xa1, 0x3f, 0x32, 0xe6, 0x52, 0xe7, 0x4b, 0xa7, 0x28, 0x57, 0x60, 0x77, 0xce,
				0x62, 0x8c, 0x53, 0x5e, 0x87, 0xdc, 0x60, 0x77, 0xba, 0x07, 0xd2, 0x90, 0x68, 0x59, 0x0c, 0x8c,
				0xb5, 0xf1, 0x08, 0x8e, 0x08, 0x2c, 0xfa, 0x0e, 0xc9, 0x61, 0x30, 0x2d, 0x69, 0xcf, 0x3d,
			},
			cipherText: []byte{
				0xdf, 0xb4, 0x40, 0xac, 0xb3, 0x77, 0x35, 0x49, 0xef, 0xc0, 0x46, 0x28, 0xae, 0xb8, 0xd8, 0x15,
				0x62, 0x75, 0x23, 0x0b, 0xdc, 0x69, 0x0d, 0x94, 0xb0, 0x0d, 0x8d, 0x95, 0xf2, 0x8c, 0x4b, 0x56,
				0x30, 0x7f, 0x60, 0xf4, 0xca, 0x55, 0xeb, 0xa6, 0x61, 0xeb, 0xba, 0x72, 0xac, 0x80, 0x8f, 0xa8,
				0xc4, 0x9e, 0x26, 0x78, 0x8e, 0xd0, 0x4a, 0x5d, 0x60, 0x6c, 0xb4, 0x18, 0xde, 0x74, 0x87, 0x8b,
				0x9a, 0x22, 0xf8, 0xef, 0x29, 0x59, 0x0b, 0xc4, 0xeb, 0x57, 0xc9, 0xfa, 0xf7, 0xc4, 0x15, 0x24,
				0xa8, 0x85, 0xb8, 0x97, 0x9c, 0x42, 0x3f, 0x2f, 0x8f, 0x8e, 0x05, 0x92, 0xa9, 0x87, 0x92, 0x01,
				0xbe, 0x7f, 0xf9, 0x77, 0x7a, 0x16, 0x2a, 0xb8, 0x10, 0xfe, 0xb3, 0x24, 0xba, 0x74, 0xc4, 0xc1,
				0x56, 0xe0, 0x4d, 0x39, 0x09, 0x72, 0x09, 0x65, 0x3a, 0xc3, 0x3e, 0x5a, 0x5f, 0x2d, 0x88,
			},
		},
		{
			// Test Set 5 – 1245 bits (155 bytes)
			name:      "NEA2TestSet5",
			key:       [16]byte{0x96, 0x18, 0xae, 0x46, 0x89, 0x1f, 0x86, 0x57, 0x8e, 0xeb, 0xe9, 0x0e, 0xf7, 0xa1, 0x20, 0x2e},
			count:     0xc675a64b,
			bearer:    0x0c,
			direction: 1,
			plainText: []byte{
				,
			},
			cipherText: []byte{
				,
			},
		},
		{
			// Test Set 6 – 3861 bits (482 bytes)
			name:      "NEA2TestSet6",
			key:       [16]byte{0x54, 0xf4, 0xe2, 0xe0, 0x4c, 0x83, 0x78, 0x6e, 0xec, 0x8f, 0xb5, 0xab, 0xe8, 0xe3, 0x65, 0x66},
			count:     0xaca4f50f,
			bearer:    0x0b,
			direction: 0,
			plainText: []byte{
				,
			},
			cipherText: []byte{
				,
			},
		},
	}

	for _, ts := range testSets {
		t.Run(ts.name, func(t *testing.T) {
			cipher, err := NEA2(ts.key, ts.count, ts.bearer, ts.direction, ts.plainText)
			if err != nil {
				t.Errorf("error: %v", err)
			}
			if !bytes.Equal(ts.cipherText, cipher) {
				t.Errorf("ciphertext mismatch: expected %x, got %x", ts.cipherText, cipher)
			}
		})
	}
}

// TestNIA2 validates 128-EIA2 (AES-CMAC-based integrity) using test vectors
// from 3GPP TS 33.401 Annex C.2.
func TestNIA2(t *testing.T) {
	t.Parallel()

	testSets := []struct {
		name      string
		key       [16]byte
		count     uint32
		bearer    uint8
		direction uint8
		message   []byte
		mac       []byte
	}{
		{
			// Test Set 2 – 64 bits (8 bytes)
			name:      "NIA2TestSet2",
			key:       [16]byte{0xd3, 0xc5, 0xd5, 0x92, 0x32, 0x7f, 0xb1, 0x1c, 0x40, 0x35, 0xc6, 0x68, 0x0a, 0xf8, 0xc6, 0xd1},
			count:     0x398a59b4,
			bearer:    0x1a,
			direction: 1,
			message:   []byte{0x48, 0x45, 0x83, 0xd5, 0xaf, 0xe0, 0x82, 0xae},
			mac:       []byte{0xb9, 0x37, 0x87, 0xe6},
		},
		{
			// Test Set 5 – 768 bits (96 bytes)
			name:      "NIA2TestSet5",
			key:       [16]byte{0x83, 0xfd, 0x23, 0xa2, 0x44, 0xa7, 0x4c, 0xf3, 0x58, 0xda, 0x30, 0x19, 0xf1, 0x72, 0x26, 0x35},
			count:     0x36af6144,
			bearer:    0x0f,
			direction: 1,
			message: []byte{
				0x35, 0xc6, 0x87, 0x16, 0x63, 0x3c, 0x66, 0xfb, 0x75, 0x0c, 0x26, 0x68, 0x65, 0xd5, 0x3c, 0x11,
				0xea, 0x05, 0xb1, 0xe9, 0xfa, 0x49, 0xc8, 0x39, 0x8d, 0x48, 0xe1, 0xef, 0xa5, 0x90, 0x9d, 0x39,
				0x47, 0x90, 0x28, 0x37, 0xf5, 0xae, 0x96, 0xd5, 0xa0, 0x5b, 0xc8, 0xd6, 0x1c, 0xa8, 0xdb, 0xef,
				0x1b, 0x13, 0xa4, 0xb4, 0xab, 0xfe, 0x4f, 0xb1, 0x00, 0x60, 0x45, 0xb6, 0x74, 0xbb, 0x54, 0x72,
				0x93, 0x04, 0xc3, 0x82, 0xbe, 0x53, 0xa5, 0xaf, 0x05, 0x55, 0x61, 0x76, 0xf6, 0xea, 0xa2, 0xef,
				0x1d, 0x05, 0xe4, 0xb0, 0x83, 0x18, 0x1e, 0xe6, 0x74, 0xcd, 0xa5, 0xa4, 0x85, 0xf7, 0x4d, 0x7a,
			},
			mac: []byte{0xe6, 0x57, 0xe1, 0x82},
		},
		{
			// Test Set 8 – 16448 bits (2056 bytes)
			name:      "NIA2TestSet8",
			key:       [16]byte{0xb3, 0x12, 0x0f, 0xfd, 0xb2, 0xcf, 0x6a, 0xf4, 0xe7, 0x3e, 0xaf, 0x2e, 0xf4, 0xeb, 0xec, 0x69},
			count:     0x296f393c,
			bearer:    0x0b,
			direction: 1,
			message: []byte{
				,
			},
			mac: []byte{0xeb, 0xd5, 0xcc, 0xb0},
		},
	}

	for _, ts := range testSets {
		t.Run(ts.name, func(t *testing.T) {
			output, err := NIA2(ts.key, ts.count, ts.bearer, ts.direction, ts.message)
			if err != nil {
				t.Errorf("error: %v", err)
			}
			if !bytes.Equal(ts.mac, output) {
				t.Errorf("MAC mismatch: expected %x, got %x", ts.mac, output)
			}
		})
	}
}

func TestConfidentialityEEA3(t *testing.T) {
	t.Parallel()

	testSets := []struct {
		name       string
		plainText  []byte
		cipherText []byte
		key        [16]byte
		count      uint32
		length     uint32
		bearer     uint8
		direction  uint8
	}{
		{
			name:      "EEA3TestSet1",
			key:       [16]byte{0x17, 0x3d, 0x14, 0xba, 0x50, 0x03, 0x73, 0x1d, 0x7a, 0x60, 0x04, 0x94, 0x70, 0xf0, 0x0a, 0x29},
			count:     0x66035492,
			bearer:    0x0f,
			direction: 0,
			length:    193,
			plainText: []byte{
				0x6c, 0xf6, 0x53, 0x40, 0x73, 0x55, 0x52, 0xab, 0x0c, 0x97, 0x52, 0xfa, 0x6f, 0x90, 0x25, 0xfe, 0x0b, 0xd6, 0x75,
				0xd9, 0x00, 0x58, 0x75, 0xb2, 0x00, 0x00, 0x00, 0x00,
			},
			cipherText: []byte{
				0xa6, 0xc8, 0x5f, 0xc6, 0x6a, 0xfb, 0x85, 0x33, 0xaa, 0xfc, 0x25, 0x18, 0xdf, 0xe7, 0x84, 0x94, 0x0e, 0xe1, 0xe4,
				0xb0, 0x30, 0x23, 0x8c, 0xc8, 0x00, 0x00, 0x00, 0x00,
			},
		},
		{
			name:      "EEA3TestSet2",
			key:       [16]byte{0xe5, 0xbd, 0x3e, 0xa0, 0xeb, 0x55, 0xad, 0xe8, 0x66, 0xc6, 0xac, 0x58, 0xbd, 0x54, 0x30, 0x2a},
			count:     0x56823,
			bearer:    0x18,
			direction: 1,
			length:    800,
			plainText: []byte{
				0x14, 0xa8, 0xef, 0x69, 0x3d, 0x67, 0x85, 0x07, 0xbb, 0xe7, 0x27, 0x0a, 0x7f, 0x67, 0xff, 0x50, 0x06, 0xc3, 0x52,
				0x5b, 0x98, 0x07, 0xe4, 0x67, 0xc4, 0xe5, 0x60, 0x00, 0xba, 0x33, 0x8f, 0x5d, 0x42, 0x95, 0x59, 0x03, 0x67, 0x51,
				0x82, 0x22, 0x46, 0xc8, 0x0d, 0x3b, 0x38, 0xf0, 0x7f, 0x4b, 0xe2, 0xd8, 0xff, 0x58, 0x05, 0xf5, 0x13, 0x22, 0x29,
				0xbd, 0xe9, 0x3b, 0xbb, 0xdc, 0xaf, 0x38, 0x2b, 0xf1, 0xee, 0x97, 0x2f, 0xbf, 0x99, 0x77, 0xba, 0xda, 0x89, 0x45,
				0x84, 0x7a, 0x2a, 0x6c, 0x9a, 0xd3, 0x4a, 0x66, 0x75, 0x54, 0xe0, 0x4d, 0x1f, 0x7f, 0xa2, 0xc3, 0x32, 0x41, 0xbd,
				0x8f, 0x01, 0xba, 0x22, 0x0d,
			},
			cipherText: []byte{
				0x13, 0x1d, 0x43, 0xe0, 0xde, 0xa1, 0xbe, 0x5c, 0x5a, 0x1b, 0xfd, 0x97, 0x1d, 0x85, 0x2c, 0xbf, 0x71, 0x2d, 0x7b,
				0x4f, 0x57, 0x96, 0x1f, 0xea, 0x32, 0x08, 0xaf, 0xa8, 0xbc, 0xa4, 0x33, 0xf4, 0x56, 0xad, 0x09, 0xc7, 0x41, 0x7e,
				0x58, 0xbc, 0x69, 0xcf, 0x88, 0x66, 0xd1, 0x35, 0x3f, 0x74, 0x86, 0x5e, 0x80, 0x78, 0x1d, 0x20, 0x2d, 0xfb, 0x3e,
				0xcf, 0xf7, 0xfc, 0xbc, 0x3b, 0x19, 0x0f, 0xe8, 0x2a, 0x20, 0x4e, 0xd0, 0xe3, 0x50, 0xfc, 0x0f, 0x6f, 0x26, 0x13,
				0xb2, 0xf2, 0xbc, 0xa6, 0xdf, 0x5a, 0x47, 0x3a, 0x57, 0xa4, 0xa0, 0x0d, 0x98, 0x5e, 0xba, 0xd8, 0x80, 0xd6, 0xf2,
				0x38, 0x64, 0xa0, 0x7b, 0x01,
			},
		},
		{
			name:      "EEA3TestSet3",
			key:       [16]byte{0xd4, 0x55, 0x2a, 0x8f, 0xd6, 0xe6, 0x1c, 0xc8, 0x1a, 0x20, 0x09, 0x14, 0x1a, 0x29, 0xc1, 0x0b},
			count:     0x76452ec1,
			bearer:    0x02,
			direction: 1,
			length:    1570,
			plainText: []byte{
				,
			},
			cipherText: []byte{
				,
			},
		},
		{
			name:      "EEA3TestSet4",
			key:       [16]byte{0xdb, 0x84, 0xb4, 0xfb, 0xcc, 0xda, 0x56, 0x3b, 0x66, 0x22, 0x7b, 0xfe, 0x45, 0x6f, 0x0f, 0x77},
			count:     0xe4850fe1,
			bearer:    0x10,
			direction: 1,
			length:    2798,
			plainText: []byte{
				,
			},
			cipherText: []byte{
				,
			},
		},
		{
			name:      "EEA3TestSet5",
			key:       [16]byte{0xe1, 0x3f, 0xed, 0x21, 0xb4, 0x6e, 0x4e, 0x7e, 0xc3, 0x12, 0x53, 0xb2, 0xbb, 0x17, 0xb3, 0xe0},
			count:     0x2738cdaa,
			bearer:    0x1a,
			direction: 0,
			length:    4019,
			plainText: []byte{
				,
			},
			cipherText: []byte{
				,
			},
		},
	}

	for _, ts := range testSets {
		t.Run(ts.name, func(t *testing.T) {
			cipher, err := NEA3(ts.key, ts.count, ts.bearer, ts.direction, ts.plainText, ts.length)
			if err != nil {
				t.Errorf("error: %v", err)
			}
			if !bytes.Equal(ts.cipherText, cipher) {
				t.Errorf("ciphertext mismatch: expected %x, got %x", ts.cipherText, cipher)
			}
		})
	}
}

func TestIntegrityEIA3(t *testing.T) {
	t.Parallel()

	testSets := []struct {
		name      string
		message   []byte
		output    []byte
		key       [16]byte
		count     uint32
		length    uint32
		bearer    uint8
		direction uint8
	}{
		{
			name:      "EIA3TestSet1",
			key:       [16]byte{0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00},
			count:     0x0,
			bearer:    0x0,
			direction: 0,
			length:    1,
			message:   []byte{0x00, 0x00, 0x00, 0x00},
			output:    []byte{0xc8, 0xa9, 0x59, 0x5e},
		},
		{
			name:      "EIA3TestSet2",
			key:       [16]byte{0x47, 0x05, 0x41, 0x25, 0x56, 0x1e, 0xb2, 0xdd, 0xa9, 0x40, 0x59, 0xda, 0x05, 0x09, 0x78, 0x50},
			count:     0x561eb2dd,
			bearer:    0x14,
			direction: 0,
			length:    90,
			message:   []byte{0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00},
			output:    []byte{0x67, 0x19, 0xa0, 0x88},
		},
		{
			name:      "EIA3TestSet3",
			key:       [16]byte{0xc9, 0xe6, 0xce, 0xc4, 0x60, 0x7c, 0x72, 0xdb, 0x00, 0x0a, 0xef, 0xa8, 0x83, 0x85, 0xab, 0x0a},
			count:     0xa94059da,
			bearer:    0x0a,
			direction: 1,
			length:    577,
			message: []byte{
				0x98, 0x3b, 0x41, 0xd4, 0x7d, 0x78, 0x0c, 0x9e, 0x1a, 0xd1, 0x1d, 0x7e, 0xb7, 0x03, 0x91, 0xb1, 0xde, 0x0b, 0x35,
				0xda, 0x2d, 0xc6, 0x2f, 0x83, 0xe7, 0xb7, 0x8d, 0x63, 0x06, 0xca, 0x0e, 0xa0, 0x7e, 0x94, 0x1b, 0x7b, 0xe9, 0x13,
				0x48, 0xf9, 0xfc, 0xb1, 0x70, 0xe2, 0x21, 0x7f, 0xec, 0xd9, 0x7f, 0x9f, 0x68, 0xad, 0xb1, 0x6e, 0x5d, 0x7d, 0x21,
				0xe5, 0x69, 0xd2, 0x80, 0xed, 0x77, 0x5c, 0xeb, 0xde, 0x3f, 0x40, 0x93, 0xc5, 0x38, 0x81, 0x00, 0x00, 0x00, 0x00,
			},
			output: []byte{0xfa, 0xe8, 0xff, 0x0b},
		},
		{
			name:      "EIA3TestSet4",
			key:       [16]byte{0xc8, 0xa4, 0x82, 0x62, 0xd0, 0xc2, 0xe2, 0xba, 0xc4, 0xb9, 0x6e, 0xf7, 0x7e, 0x80, 0xca, 0x59},
			count:     0x05097850,
			bearer:    0x10,
			direction: 1,
			length:    2079,
			message: []byte{
				,
			},
			output: []byte{0x00, 0x4a, 0xc4, 0xd6},
		},
		{
			name:      "EIA3TestSet5",
			key:       [16]byte{0x6b, 0x8b, 0x08, 0xee, 0x79, 0xe0, 0xb5, 0x98, 0x2d, 0x6d, 0x12, 0x8e, 0xa9, 0xf2, 0x20, 0xcb},
			count:     0x561eb2dd,
			bearer:    0x1c,
			direction: 0,
			length:    5670,
			message: []byte{
				,
			},
			output: []byte{0x0c, 0xa1, 0x27, 0x92},
		},
	}

	for _, ts := range testSets {
		t.Run(ts.name, func(t *testing.T) {
			output, err := NIA3(ts.key, ts.count, ts.bearer, ts.direction, ts.message, ts.length)
			if err != nil {
				t.Errorf("error: %v", err)
			}
			if !bytes.Equal(ts.output, output) {
				t.Errorf("MAC mismatch: expected %x, got %x", ts.output, output)
			}
		})
	}
}
